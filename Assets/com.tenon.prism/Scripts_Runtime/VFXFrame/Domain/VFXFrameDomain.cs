...
namespace TenonKit.Prism {
    ...
    internal static class VFXFrameDomain {
        ...
        internal static bool TryPlayManualy(VFXFrameContext ctx, int vfxID) {
            ...
            entity.state = VFXFrameState.Playing;
            return true;
        }
        ...
        internal static int TryPreSpawnVFX_ToTarget(VFXFrameContext ctx, string vfxName, Sprite[] frames, bool isLoop, float frameInterval, Transform attachTarget, Vector3 offset) {
            return TrySpawnAndDelayPlayVFX(ctx, vfxName, frames, isLoop, frameInterval, attachTarget, offset, VFXFrameState.Stop);
        }
        ...
        internal static int TryPreSpawnVFX_ToWorldPos(VFXFrameContext ctx, string vfxName, Sprite[] frames, bool isLoop, float frameInterval, Vector3 pos) {
            return TrySpawnAndDelayPlayVFX(ctx, vfxName, frames, isLoop, frameInterval, null, pos, VFXFrameState.Stop);
        }
        ...
        internal static int TrySpawnAndPlayVFX_ToTarget(VFXFrameContext ctx, string vfxName, Sprite[] frames, bool isLoop, float frameInterval, Transform attachTarget, Vector3 offset) {
            return TrySpawnAndDelayPlayVFX(ctx, vfxName, frames, isLoop, frameInterval, attachTarget, offset, VFXFrameState.Playing);
        }
        ...
        internal static int TrySpawnAndPlayVFX_ToWorldPos(VFXFrameContext ctx, string vfxName, Sprite[] frames, bool isLoop, float frameInterval, Vector3 pos) {
            return TrySpawnAndDelayPlayVFX(ctx, vfxName, frames, isLoop, frameInterval, null, pos, VFXFrameState.Playing);
        }

        // 生成, 并播放
        static int TrySpawnAndDelayPlayVFX(VFXFrameContext ctx, string vfxName, Sprite[] frames, bool isLoop, float frameInterval, Transform attachTarget, Vector3 offset, VFXFrameState state) {
            if (vfxName == null) {
                return -1;
            }

            if (!TrySpawnVFX(ctx, vfxName, frames, isLoop, frameInterval, state, out var entity)) {
                return -1;
            }
        ...
        static bool TrySpawnVFX(VFXFrameContext ctx, string vfxName, Sprite[] frames, bool isLoop, float frameInterval, VFXFrameState state, out VFXFramePlayerEntity entity) {

            var repo = ctx.Repo;
            entity = VFXFrameFactory.SpawnVFXPlayer(ctx, vfxName, frames, isLoop, frameInterval, state);
